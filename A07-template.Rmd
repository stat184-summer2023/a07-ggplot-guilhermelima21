...
```{r}
### Your code here
Trips %>% ggplot(aes(x=client, y=(as.numeric(edate - sdate)/60))) + geom_boxplot() + ylim(0,60) +labs(y="dur/60") + theme_linedraw()
...
```{r}
### Your Code Here
Trips %>% ggplot(aes(x= lubridate::yday(sdate))) + geom_density(fill = "gray") + labs(x="Day of Year", y= "density") + theme_linedraw()
```


My observations here..... 

Bikes were checked out the most during October, waning in November, then a slight increase during December then dropping again.

### Day of Week

```{r}
### Your Code Here
Trips %>% ggplot(aes(x= lubridate::wday(sdate))) + geom_density(fill = "gray") + labs(x="Day of Week", y= "density") + theme_linedraw()
```


My observations here..... 

Bikes were checked out in similar quantities throughout the week. The bikes are rented less at night, and Thursday & Friday are when bikes are used the most. 


### Hour of Day

```{r}
Trips %>% ggplot(aes(x= lubridate::hour(sdate))) + geom_density(fill = "gray") + labs(x="Hour of Day", y= "density") + theme_linedraw()
...
Bikes are checked out more frequently between 6am-9am, dropping around noontime, then frequently again during 3pm-8pm. The bikes are not checked out very frequently before 5am or after 8pm. 


### Minute of Hour

```{r}
Trips %>% ggplot(aes(x= lubridate::minute(sdate))) + geom_density(fill = "gray") + labs(x="Minute of Hour", y= "density") + theme_linedraw()
...
Bikes are rented frequently throughout the minute of the hour. Although there is a slight dip at 0min and 60min, looking at the y-axis indicates that the scale is very small and thus the dip is insignificant. 
...
```{r}
### My Code here
Trips %>% mutate(hour = lubridate::hour(sdate), weekday = lubridate::wday(sdate)) %>% group_by(hour,weekday,client) %>% summarise(totalEvents = n()) %>% ggplot() +
  aes(
    x = hour,
    y = totalEvents,
    colour = weekday,
  ...
  theme_linedraw() +
  facet_wrap(vars(client)) + labs(x="Hour of Day", y= "Total Number of Events")

```

My comments here.....

We notice that casual clients check out bikes less frequently than registered clients. Registerd clients have a much higher spike in bikes checked out during ~8am and ~4-6pm. From the weekday grouping, we can observe that casual clients rent bikes more frequently on the Sunday since the yellow dots are on top of the rest, while registered clients rent them on a mix of weekdays.
...
### My code here
Trips %>%
  inner_join(PairDistances, by = c('sstation', 'estation')) %>% ggplot(aes(x = distance)) + geom_density(color = 'blue') + geom_density(data=PairDistances,alpha = 0.2,color = NA,fill='red') + xlab('Trip Distance (km)') + xlim(0,15) + theme_linedraw()
```
The graphs for the distribution of ride distances compared to pairwise differences of stations is similar. We observe that the density for ride distances is mostly centered around 1-3km, while pairwise distance between stations is spread across 0-15km. 
...
We need duplicate data with different table names so that we can calculate the latitude and longitude distances with haversine(). We need two pairs of (lat,long) to do a distance calculation. 
